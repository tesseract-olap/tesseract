...
pub fn agg_sql_string_pass_1(col: &str, aggregator: &Aggregator, mea_idx: usize) -> String {
    ...
    match aggregator {
        ...
        Aggregator::Median => format!("{} as m{}", col, mea_idx),
        Aggregator::GroupedMedian { sub_aggregator } => {
            format!("{}({}) as m{}", sub_aggregator.sql_string(), col, mea_idx)
        },
        ...
        Aggregator::WeightedSum { weight_column } => {
            format!("sum({0} * {1}) as m{2}_weighted_sum_pass_1",
                col,
                weight_column,
                mea_idx,
            )
...
pub fn agg_sql_string_select_mea(aggregator: &Aggregator, mea_idx: usize) -> String {
    match aggregator {
        ...
        Aggregator::GroupedMedian { .. } => format!("m{0}", mea_idx),
        ...
        Aggregator::WeightedSum { .. } => {
            format!("m{0}_weighted_sum_pass_1",
                mea_idx,
            )
        },
        Aggregator::Moe { secondary_columns, .. }=> {
...
pub fn agg_sql_string_pass_2(aggregator: &Aggregator, mea_idx: usize) -> String {
    ...
    match aggregator {
        ...
        Aggregator::GroupedMedian { .. } => format!("median(m{0}) as final_m{0}", mea_idx),
        Aggregator::WeightedAverage { .. } => {
        ...
        Aggregator::WeightedSum { .. } => {
            format!("sum(m{0}_weighted_sum_pass_1) as final_m{0}",
                mea_idx,
            )
        },
